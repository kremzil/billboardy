...
- `/inquiries` принимает публичный POST из dopyt-виджета frontend, валидирует контактные поля и выбранные плоскости, затем отправляет письмо через `wp_mail` на адрес из настройки плагина `E-mail pre dopyty`; если настройка пустая или невалидная, используется admin email сайта.
