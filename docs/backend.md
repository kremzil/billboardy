...
- `/inquiries` принимает публичный POST из dopyt-виджета frontend, валидирует контактные поля и выбранные плоскости, затем отправляет письмо через `wp_mail` на admin email сайта.
