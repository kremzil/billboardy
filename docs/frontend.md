...
`mhdAdvertising.ts` хранит недисконтированные официальные исходные цены DPMK на 2026 год. Для отображения применяется `discountPrice(sourcePrice)`: 97% от исходной цены с округлением до центов. Все показанные цены указаны без 23% DPH; временный polep в карточках автобусов и электричек стоит 100 € без DPH. В таблицах SIT/VLAJKA эта строка не отображается.

Блок `Verejné osvetlenie` построен в две колонки: слева находятся заголовок и таблицы SIT/VLAJKA, справа — интерактивная панель официальной категоризации улиц DPMK. Табы A, B и C переключают прокручиваемые списки; активный таб расширяется, а два неактивных остаются узкими. На мобильных устройствах панель переносится под таблицы.
