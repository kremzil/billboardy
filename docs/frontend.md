...
Локальная навигация `Autobusy / Električky / Verejné osvetlenie` размещена после обзорного блока и при прокрутке становится липкой. Полоса использует цвет `bb-control`, поэтому светлые контрастные чипы хорошо отделяют обзор от подробного каталога.
