...
`inquiry.ts` управляет sessionStorage-выборкой точек, bubble-счётчиком, modal-формой и POST-запросом на `/inquiries`. При чтении старых данных из sessionStorage выбранные точки нормализуются: запись без ID отбрасывается, а пустое название восстанавливается из кода, адреса или ID. Modal-форма имеет focus-trap (циклический Tab внутри диалога), закрытие по Escape и возврат фокуса на элемент-инициатор при закрытии; счётчик выбранных плоскостей объявляется screen reader'ам через `aria-live="polite"`.
