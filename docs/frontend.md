...
В блоке `Verejné osvetlenie` заголовок и вводный текст вынесены отдельно над контентом. Ниже в две колонки выровнены таблицы SIT/VLAJKA и интерактивная карточка официальной категоризации улиц DPMK со стандартным красным заголовком. Табы A, B и C переключают прокручиваемые списки; активный таб расширяется, два неактивных остаются узкими, а при наведении используется такой же нейтральный `shadow-md`, как у карточек форматов. На мобильных устройствах панель переносится под таблицы.
